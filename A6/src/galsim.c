...
void update_particles(particle_t* particles, vector_t* forces, int N, double delta_t) {
    #pragma omp parallel for // <--- WTF, y u slwr
    for(int i = 0; i < N; i++) {
...
int main(int argc, char* argv[]) {
    ...
    double timeparty = 0.0;

    for(int i = 0; i < nsteps; i++) {
        tree = NULL;
        double time2 = get_wall_seconds();
        quadtree_build_tree(&tree, particles, N);
        double time3 = get_wall_seconds();
        compute_quad_forces(forces, tree, particles, N, theta_max);
        ...
        timeparty += time6 - time5;
    }
    printf("\n");
    double time7 = get_wall_seconds();
    printf("quadtime: %f \nforcetime: %f \n partytime %f\n", timequad, timeFORCE, timeparty);
